...
class FiMCPService:
    ...
    async def verify_authentication(self, session_id: str, mobile_number: str, otp: str) -> Dict[str, Any]:
        ...
        try:
            logger.info("🔐 Verifying Fi MCP authentication", extra={"mobile_number": mobile_number})
            ...
            try:
                ...
                if login_response.status_code == 200:
                    ...
                    logger.info("✅ Fi MCP authentication successful", extra={
                        "mobile_number": mobile_number,
            ...
            except httpx.RequestError as login_error:
                logger.error("❌ Login request failed", extra={"error": str(login_error)})
                return {"success": False, "message": f"Login failed: {str(login_error)}"}

        except Exception as e:
            logger.error("❌ Fi MCP authentication verification failed", extra={"error": str(e)})
